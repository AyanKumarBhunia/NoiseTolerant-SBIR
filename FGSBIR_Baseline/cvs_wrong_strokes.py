...
def Wrong_Strokes_FGSBIR(model, datloader_Test):
    ...
    for i_batch, sanpled_batch in enumerate(datloader_Test):
        ...
        for i in range(len(rank_sketch)):
            if i <= min_locn:
                strokes_to_keep.append(i)

        print('old_length', get_stroke_num(sketch_coord))

        new_strokes_coordinates = select_strokes(sketch_coord, strokes_to_keep)

        print('new_length', get_stroke_num(new_strokes_coordinates))

        # print(sanpled_batch['sketch_path'][0], new_strokes_coordinates)

        print(sanpled_batch['sketch_path'][0])

        new_strokes_dict.update({sanpled_batch['sketch_path'][0]: new_strokes_coordinates})
    save_path = 'cvs_dataset/' + hp.cvs
