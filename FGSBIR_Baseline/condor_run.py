...
with condor('condor') as sess: #aisurrey-condor vs condor

    run_file = f'{os.getcwd()}/cvs_train_main.py'
    exp_name = 'FGSBIR_Augmentation'
    folder_name = '_'.join([exp_name, datetime.now().strftime("%b-%d_%H:%M:%S")])

    for bs in ['Top_10','Top_1']: # submit a bunch of jobs
