...
from rasterize import mydrawPNG_fromlist, get_stroke_num, mydraw_redPNG_fromlist, convert_to_red, convert_to_blue, \
    convert_to_green, convert_to_black
...
def Wrong_Strokes_FGSBIR(model, datloader_Test):
    ...
    for i_batch, sanpled_batch in enumerate(datloader_Test):
        ...
        # red_strokes[0] = False
        min = torch.tensor(99999)
        last = distances[0]

        print('rank_sketch', torch.stack(rank_sketch))

        min_locn = len(rank_sketch) - 1 - torch.flip(torch.stack(rank_sketch), [0]).argmin()
        for i in range(len(rank_sketch)):
            if i <= min_locn:
                colored_strokes.append(0)
            else:
                colored_strokes.append(1)
        #
        # if torch.stack(rank_sketch).min() > torch.stack(rank_sketch)[-1]:
        #     colored_strokes = [0] * len(colored_strokes)
        ...
        print('red_strokes', colored_strokes)

        complete_sketch_stroke = stroke_combi[-1]
        full_sketch_image = mydraw_redPNG_fromlist(sketch_coord, complete_sketch_stroke)


        new_sketch_image = [mydraw_redPNG_fromlist(sketch_coord, x) for x in stroke_for_image]
        new_sketch_image_incomplete = [mydraw_redPNG_fromlist(sketch_coord, x) for x in stroke_for_image]

        for i in range(len(colored_strokes)):
            if colored_strokes[i] == 1:
                new_sketch_image[i] = convert_to_red(new_sketch_image[i])
            if colored_strokes[i] == 2:
                new_sketch_image[i] = convert_to_blue(new_sketch_image[i])
            if colored_strokes[i] == 3:
                new_sketch_image[i] = convert_to_green(new_sketch_image[i])

        for i in range(len(colored_strokes)):
            if colored_strokes[i] == 1:
                new_sketch_image_incomplete[i] = convert_to_black(new_sketch_image_incomplete[i])

        sketch_image = np.array(new_sketch_image)
        sketch_image = np.sum(sketch_image, axis=0).clip(0,255)

        sketch_image_incomplete = np.array(new_sketch_image_incomplete)
        sketch_image_incomplete = np.sum(sketch_image_incomplete, axis=0).clip(0, 255)


        a = torch.tensor(full_sketch_image)
        b = torch.tensor(sketch_image)
        c = torch.tensor(sketch_image_incomplete)

        save_image(torch.cat((a, b, c), 2), random_string(i_batch))
        print('Image saved')
