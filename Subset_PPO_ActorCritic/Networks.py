...
class Sketch_Value_Network(nn.Module):
    def __init__(self, hp):
        super(Sketch_Value_Network, self).__init__()
        ...
        self.value_predictor = nn.Linear(hp.hidden_size, 1)
    ...
    def forward(self, batch):
        ...
        for x, y in zip(x_sketch_h, x_sketch_hidden):
            out.append(self.layernorm(x + y))
        output_hidden = pad_sequence(out, batch_first=True) # to make same number of strokes for each sketch

        embedding_pool, _ = output_hidden.max(1)
        value_output = self.value_predictor(embedding_pool)
