...
if __name__ == "__main__":
    ...
    parser.add_argument('--saved_models', type=str,
                        default='./models', help='Saved models directory')
    ...
    parser.add_argument('--print_freq_iter', type=int, default=1000)
    parser.add_argument('--debug', type=bool, default=False)
    parser.add_argument('--reward', type=str, default='rank1',
                        help='rank1 / rank2 / rank3 / rank4 / rank5 / rank6 / rank7 / rank8')
    parser.add_argument('--margin', type=float, default=0.1)
    parser.add_argument('--distribution', type=str, default='categorical',
                        help='categorical / binomial / bernoulli')
    parser.add_argument('--eval', type=bool,
                        default=False, help='False / True')
    ...
    reward, policy_loss, mse_loss, top1_train, top10_train, avg_train = 0, 0, 0, 0, 0, 0
    ...
    for i_epoch in range(hp.max_epoch):
        ...
        for batch_data in dataloader_Train:
            ...
            start = time.time()
            model.train()
            # reward, policy_loss, top1_train, top10_train, avg_train =
            loss_all, policy_loss, mse_loss = model.train_RL_selector(
                batch=batch_data)
            ...
            if step_count % hp.print_freq_iter == 0:
                print(
                    'Epoch: {}, Iteration: {}, Loss All: {:.5f}, Policy Loss: {:.5f}, MSE Loss: {:.5f}, Time: {}'.format
                    (i_epoch, step_count, loss_all, policy_loss, mse_loss, time.time() - start))

            if step_count % hp.eval_freq_iter == 0:
                with torch.no_grad():
                    top1_eval, top10_eval, avg_eval = model.evaluate(
                        dataloader_Test)
                    print('evaluation results top1',
                          top1_eval)
        ...
        # print('training results top1 / top10 / avg ranks : ',
        #       top1_train_avg, ' / ', top10_train_avg, '/', avg_train_avg)
