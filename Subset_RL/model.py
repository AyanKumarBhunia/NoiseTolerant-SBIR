...
class FGSBIR_Model(nn.Module):
    ...
    def train_RL_selector(self, batch):
        ...
        if self.hp.reward == 'rank1':
            ...
            reward = - triplet_loss_reward + 1 / torch.sqrt(torch.abs(rank_reward).float())
        elif self.hp.reward == 'rank7':
            reward = - torch.sqrt(torch.abs(triplet_loss_reward))
        elif self.hp.reward == 'rank8':
            reward = - torch.sqrt(torch.abs(triplet_loss_reward).float()) + 1 / torch.sqrt(torch.abs(rank_reward).float())
        elif self.hp.reward == 'rank9':
            reward = - 10 * triplet_loss_reward - rank_reward
        elif self.hp.reward == 'rank10':
            reward = - 100 * triplet_loss_reward - rank_reward
        elif self.hp.reward == 'rank11':
            reward = - 1000 * triplet_loss_reward - rank_reward
        elif self.hp.reward == 'rank12':
            reward = - 0.1 * triplet_loss_reward - rank_reward
        elif self.hp.reward == 'rank13':
            reward = - 0.01 * triplet_loss_reward - rank_reward
        elif self.hp.reward == 'rank14':
            reward = - 0.001 * triplet_loss_reward - rank_reward
